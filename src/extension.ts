...
export class Ext extends Ecs.System<ExtEvent> {
    ...
    on_active_workspace_changed() {
        ...
        const refocus_hint = () => {
            if (!this.active_hint?.tracked || this.active_hint.tracked.kind !== 1) return;
            ...
            if (aws[0] === cws[0] && aws[1] === cws[1]) {
                this.active_hint.track_window(this, active);
            } else {
    ...
    on_overview_hidden() {
        if (this.active_hint) {
            this.active_hint.overview_shown = false;
    ...
    on_overview_shown() {
        if (this.active_hint) {
            ...
            this.active_hint.overview_shown = true;
        }
